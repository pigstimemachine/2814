...
{ 	
	...
	{	 		   
		Timer0_IsrHandler();
        //Twinkle3Time(50);
        RightKey_LongPress_Process();
        LeftKey_LongPress_Process();
        doublekey_press_process();
		TIM2->SR&=~(1<<0);		//清除中断标志位		   
	}	    
}
...
/*-----------------------------------------------------------------
函数名称: void TIM3_Int_Init(unsigned int arr,unsigned int psc)
函数功能: 通用定时器3中断初始化	  
...
void TIM3_Int_Init(unsigned int arr,unsigned int psc)
{
  ...
  MY_NVIC_Init(2,3,TIM3_IRQChannel,2);//抢占1，子优先级3，组2	
}
