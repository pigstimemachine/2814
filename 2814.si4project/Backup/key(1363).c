...
KEY_PARA LayerKeyLeft;
KEY_PARA LayerKeyRight;
...
{
    if( (LayerKeyRight.sts == PressDown) && ( LayerKeyLeft.sts == PressDown ) )
    {
...
int RightKey_LongPress_Process(void)
{
    ...
    if( LayerKeyRight.sts == PressDown )
    {
        LayerKeyRight.time ++ ;
    ...
    {
        LayerKeyRight.time  = 0 ;
    }
    if( LayerKeyRight.time > LongPressTimeSts )
    {
        layer_time++;
...
int LeftKey_LongPress_Process(void)
{
    ...
    if( LayerKeyLeft.time > LongPressTimeSts )
    {
        layer_time++;
        if( layer_time > ButtonChangeSpeed )
        {
