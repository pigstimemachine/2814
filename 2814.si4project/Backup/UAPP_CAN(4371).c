...
DataBufType DecodeData,CAN_CodePipe,CAN_CodeWelder;
...
void _WRITEDecodeData(u8* Databuf,u8 BufCtr,u8 DLC,u8 Readflag,DataBufType codedata)
{
	 if(Readflag!=NRead)
	 {	 
	   memcpy(&codedata.CodeData[BufCtr],Databuf,DLC);
	 }	 
	 else
	 {	 
	   codedata.Readflag=Readflag;
		 codedata.DLC=DLC;
	 } 
}
...
{
	 memset(&CAN_Trans,0,sizeof(CAN_Trans));
...
u8 _Rx_CodeData (CANBufType* buf,u8 Addr,TRANSMIT_CTRL CAN_Trans,DataBufType codedata)
{
	 u8 DLC,cnt,Tmpcnt;
	 DLC=buf->DLC;
	...
	// _CommWatchDog_Feed(Addr);
	 ...
	 {	 
		 ...
		 DLC-=2;
		 _WRITEDecodeData(&buf->FrameData[2],CodeTransCtrl.Datacnt,DLC,0,codedata);
		    ...
		    if(CAN_Trans.Tmpsum==CAN_Trans.Datasum)
				{	
					...
					// STMFLASH_Write(STM32_FLASH_BASE+ADDR_PARA_OFFSET,(u16*)MEM_Databuf,MAX_PARA_NUM);                     
					 ...
					 _WRITEDecodeData(&buf->FrameData[2],CAN_Trans.Datacnt,CAN_Trans.Datacnt,NRead,codedata);
...
//      break;
      ...
      case CMD_PipeApply://
         Ackflag=_Rx_CodeApply(CANRxData.FrameData,Addr,CAN_CodePipe);
      break;
    case CMD_Pipe:
        Ackflag = _Rx_CodeData(&CANRxData,Addr,CAN_CodePipe);
      break;    
...
void CAN_TASK1000ms(void)  //1HZ
{
	...
	LCDShowStringUpdate(45,20,DecodeData.CodeData,His_AS4Data.WELD_CODE,60,RED,WHITE,16,0);
