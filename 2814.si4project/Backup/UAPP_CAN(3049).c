...
void _WRITEDecodeData(u8* Databuf,u8 BufCtr,u8 DLC,u8 Readflag)
{
	 if(Readflag!=NRead)
	 {	 
	   memcpy(&CAN_DataPipe.CodeData[BufCtr],Databuf,DLC);
	 }	 
	 else
	 {	 
	   CAN_DataPipe.Readflag=Readflag;
		 CAN_DataPipe.DLC=DLC;
	 } 
}

u8 _Rx_CodeApply(u8* buf,u8 Addr)
{
	 memset(&CAN_CodeTrans_Pipe,0,sizeof(CAN_CodeTrans_Pipe));
	 memcpy(&(CAN_CodeTrans_Pipe.Datalen),&buf[1],3);
	 memcpy(&(CAN_CodeTrans_Pipe.Datasum),&buf[4],4);
	 CAN_CodeTrans_Pipe.Tmpsum=0x10000000;
...
u8 _Rx_CodeData (CANBufType* buf,u8 Addr)
{
	 u8 DLC,cnt,Tmpcnt;
	 DLC=buf->DLC;
	...
	// _CommWatchDog_Feed(Addr);
	 if((CAN_CodeTrans_Pipe.Framecnt&0xff)==Tmpcnt)
	 {	 
		 for(cnt=2;cnt<DLC;cnt++)
		 {
				CAN_CodeTrans_Pipe.Tmpsum-=buf->FrameData[cnt];
		 } 
		 DLC-=2;
		 _WRITEDecodeData(&buf->FrameData[2],CAN_CodeTrans_Pipe.Datacnt,DLC,0);
		 CAN_CodeTrans_Pipe.Datacnt+=DLC;
		 if(CAN_CodeTrans_Pipe.Datacnt>=CAN_CodeTrans_Pipe.Datalen)
		 {
		    if(CAN_CodeTrans_Pipe.Tmpsum==CAN_CodeTrans_Pipe.Datasum)
				{	
					...
					// STMFLASH_Write(STM32_FLASH_BASE+ADDR_PARA_OFFSET,(u16*)MEM_Databuf,MAX_PARA_NUM);                     
					 ...
					 _WRITEDecodeData(&buf->FrameData[2],CAN_CodeTrans_Pipe.Datacnt,CAN_CodeTrans_Pipe.Datacnt,NRead);
...
//     case RFCMD_SendCode:
//             Ackflag=_Rx_CodeApply(TmpRxData,Addr);
//             break;
//     case RFCMD_SendCodeData:
//             _Rx_CodeData(TmpRxData,Addr);
//             break; 

//      case CMD_CodeApply://0x30应答发送扫码数据申请
//         Ackflag=_Rx_CodeApply(CANRxData.FrameData,Addr);
//      break;
//    case CMD_CodeData://应答发送扫码数据
//        Ackflag = _Rx_CodeData(&CANRxData,Addr);
//      break;
