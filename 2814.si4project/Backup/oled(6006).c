...
int LCDShowStringUpdate(const char* NowString,const char* HisString,u16 x,u16 y,const u8 *p,u16 fc,u16 bc,u8 sizey,u8 mode)
{
	if(memcmp( NowString,HisString ,sizeof(NowString)) != 0)
		{
			LCD_ShowString(x, y,  *p, bc, bc, sizey, mode);
			memcpy(HisString  ,NowString,sizeof(NowString) );
		}
	LCD_ShowString(x, y,  *p, bc, bc, sizey, mode);
	return 0;
}
