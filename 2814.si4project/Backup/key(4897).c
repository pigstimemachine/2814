...
{
    if( (Right_key.sts == PressDown) && ( Left_key.sts == PressDown ) )
    {
...
int RightKey_LongPress_Process(void)
{
    ...
    if( Right_key.time > LongPressTimeSts )
    {
        layer_time++;
...
int LeftKey_LongPress_Process(void)
{
    ...
    if( Left_key.time > LongPressTimeSts )
    {
        layer_time++;
        if( layer_time > ButtonChangeSpeed )
        {
