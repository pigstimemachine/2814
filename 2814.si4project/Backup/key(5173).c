...
KEY_PARA LayerKeyMinus,LayerKeyAdd,FunKeyLeft,FunKeyRight;
...
{
    if( (LayerKeyAdd.sts == PressDown) && ( LayerKeyMinus.sts == PressDown ) )
    {
...
int RightKey_LongPress_Process(void)
{
    ...
    if( LayerKeyAdd.time > LongPressTimeSts )
    {
        layer_time++;
...
{
    static int layer_time ;
    if( LayerKeyMinus.sts == PressDown )
    {
        LayerKeyMinus.time ++ ;
    }
    else
    {
        LayerKeyMinus.time  = 0 ;
    }
    if( LayerKeyMinus.time > LongPressTimeSts )
    {
        layer_time++;
        if( layer_time > ButtonChangeSpeed )
        {
...
{
    if( ( PBin(0) ) == 0) //按键right KEY2
    {
		LayerKeyAdd.sts = 1;
		Interface.page = PageAbout;
        LayerKeyAdd.cnt++;
        if( (can_layer.rcv_sts == 1 ) && can_layer.send_sts == SendDataSUCC )
        {
            can_layer.layer++;
